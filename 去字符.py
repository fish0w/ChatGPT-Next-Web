...
def remove_chinese_chars(input_str):
    # 使用正则表达式匹配所有中文字符
    result = re.sub(r'[\u4e00-\u9fff]+', '', input_str)
    print(result)
    return result


def 提取():
    pattern = r'avatar:\s*"(.*?)",\s*name'
    message = """export const CN_MASKS: BuiltinMask[] = [
  { 
...
def imageprocess(img_url,x,y):
    ...
    # 保存新尺寸的图片
    resized_image.save(img_url[:img_url.rfind(".")]+f"-{x}*{y}.png", 'PNG')
...
def resize_svg(input_svg_path, new_width, new_height):
    # 读取 SVG 文件
    parser = etree.XMLParser(recover=True)
    tree = etree.parse(input_svg_path, parser)

    # 获取 SVG 根元素并更改宽度和高度属性
    root = tree.getroot()
    root.attrib['width'] = str(new_width)
    root.attrib['height'] = str(new_height)

    # 构造新的 SVG 文件路径
    directory, filename = os.path.split(input_svg_path)
    filename_without_extension = os.path.splitext(filename)[0]
    output_svg_path = os.path.join(directory, f"{filename_without_extension}_{new_width}x{new_height}.svg")

    # 保存更改后的 SVG 文件
    with open(output_svg_path, 'wb') as file:
        tree.write(file)
...
if __name__ == '__main__':
    # 提取()
    image="/Users/fish052/Downloads/用所选项目新建的文件夹 3/bot.svg"
    # imageprocess(image,43,44)
    # imageprocess(image, 120, 120)

    output_path = resize_svg(image, 16, 16)
    print(f"SVG file resized and saved at {output_path}")
